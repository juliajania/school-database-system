...
SELECT P1.*, NumberOfHours
FROM Persons AS P1
JOIN
(
SELECT CL.Student_ID, SUM(Hours) AS NumberOfHours
...
SELECT *, FORMAT ( CAST (O2.liczba_nieobecnosci AS DECIMAL(5, 2)) / (  liczba_nieobecnosci + O1.liczba_obecnosci) , 'P')
FROM Class AS C
JOIN
(SELECT CL.Class_ID, COUNT(*) AS liczba_obecnosci
FROM Absences AS A
...
JOIN
(SELECT CL.Class_ID, COUNT(*) AS liczba_nieobecnosci
FROM Absences AS A
JOIN Classroom AS CL ON A.Classroom_ID = CL.Classroom_ID
WHERE Status = 'Absent'
GROUP BY CL.Class_ID)
...
SELECT Class_Name, C2.Class_ID, œrednia_ocena_w_klasie
FROM Class AS C1
JOIN
(SELECT C.Class_ID, AVG(ER.Mark) AS œrednia_ocena_w_klasie
FROM Class AS C 
...
SELECT P.*, CO1.Name, CO2.Student_ID, CO2.Course_ID, œrednia
FROM Course AS CO1
JOIN 
(SELECT ER.Student_ID, CO.Course_ID, AVG(ER.Mark) AS œrednia
FROM Course AS CO
...
SELECT L.*, liczba_klas 
FROM Language AS L
JOIN
(SELECT CO.Language_ID, COUNT(C.Class_Name) AS liczba_klas 
FROM Course AS CO 
JOIN Class AS C ON C.Course_ID = CO.Course_ID
GROUP BY CO.Language_ID) AS L1 ON L.Language_ID = L1.Language_ID
 ...
 SELECT CL.Student_ID, COUNT(Status) AS number_of_unjustified_absences
 FROM Absences AS A
 JOIN Classroom AS CL ON A.Classroom_ID = CL.Classroom_ID
 WHERE Status = 'Absent'
 GROUP BY CL.Student_ID  
 ...
 SELECT P.*, Teacher_ID, Exam_ID, AverageMark
 FROM Persons AS P
 JOIN
 (SELECT Teacher_ID, Exam_ID, AVG(Mark) AS AverageMark
 FROM Exams_Results AS ER
